...
#plot and wrap
p_saa <- ggplot(merged_T, aes(MND, expression, fill=SEX)) + 
  geom_boxplot() + 
  facet_wrap(~external_gene_name, scales="free_y") + 
  labs(x="ALS status", 
       y="Expression (log normalised counts)", 
...
#human search
human_db <- search_kegg_organism('hsa', by='kegg_code')
...
```{r}
x<- top_50_genes$external_gene_name
#x <- c("SAA2", "SAA1", "SAA2-SAA4")
x
brit <- bitr(x, "SYMBOL", "ENTREZID", OrgDb = org.Hs.eg.db)
y <- brit$ENTREZID
#y <- c("6289", "6288", "100528017")
y <- as.character(y)
yy <- bitr_kegg(y, fromType = "kegg", toType = "Module", organism = "hsa")
yy

keggLink("pathway", c("hsa:5214", "hsa:3706"))

pathways <- keggList("pathway", "hsa")
head(pathways)

map <- gsub("path:", "", names(pathways)[2])
map# remove 'path:'
pv.out <- pathview(gene.data = gene.data, cpd.data = y, gene.idtype = "KEGG", 
